...
The [[Authorization Flows - Attack]] was originally designed for native or single-page apps that cannot securely store Client Credentials. However, its use is now discouraged and is not included in the OAuth 2.1 specification. Despite this, it is still a viable authentication solution within Open ID Connect (OIDC) to retrieve `id_tokens`.
...
![Device Authorization Flow](https://blog.doyensec.com/public/images/oauth-device-authorization-flow.png)
### References
